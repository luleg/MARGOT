...
int ReadArgs(int argc, char **argv){
  ...
  while( num_arg < argc){
    ...
    else if (strcmp(argv[num_arg], "-orb") == 0){ // output file where the MAM edgelist will be stored
      num_arg ++;

      while(num_arg < argc and argv[num_arg][0]!='-')  {
        Orbits.AddUnique(atoi(argv[num_arg]));
        num_arg ++;
      }
      num_arg--;
    }
...
void BuildMAM(TStr InGraph,TStr Motif, Graph& MAMGraph, TStr OutMAM){
  ...
  if (type_mot == 'q'){// The Motif is a quadrangle::
    ...
    if (nbProc>0){ // If Several processors
      ...
      ExeTm.Tick();
      Benson = new BensonGraphQuadMP(SGraph,QuadMot,Orbits,nbProc, verbose);
      TStart = TSecTm::GetCurTm()-TStart;
      if (verbose){
    ...
    /// SEQUENTIAL
      ...
      ExeTm.Tick();
      Benson = new BensonGraphQuad(SGraph,QuadMot,Orbits,true, verbose);
      TStart = TSecTm::GetCurTm()-TStart;
      if (verbose){
        fprintf(stderr,":::CPU Time to build the MAM\t:%s\n",ExeTm.GetTmStr());
      }
  ...
  ////////// TRIANGLE
    ...
    if (nbProc>0){
      TStart = TSecTm::GetCurTm();
      ExeTm.Tick();
      Benson = new BensonGraphTriangleMP(SGraph,TriMot,Orbits,nbProc, verbose);
      TStart = TSecTm::GetCurTm()-TStart;
      if (verbose){
        fprintf(stderr,":::CPU Time to build the MAM\t:%s\n(Ellapsed Time\t\t:%s)\n",
                ExeTm.GetTmStr(),TStart.GetTmStr().CStr());
    ...
    /// SEQUENTIAL
      ...
      ExeTm.Tick();
      Benson = new BensonGraphTriangle(SGraph,TriMot,Orbits,true, verbose);
      TStart = TSecTm::GetCurTm()-TStart;
      if (verbose){
        fprintf(stderr,":::CPU Time to build the MAM\t:%s\n",ExeTm.GetTmStr());
      }
  ...
  if (not OutMAM.Empty()){ // If the MAM is required to be saved in a text file
    ExeTm.Tick();
...
void usage(){
  ...
  if (buildMAM && partition && postproc){
    ...
    printf("-orb 1 2 ...\t:Indicates the orbits to use as anchors.\n");
    printf("\tGiven a graphlet id, the orbits are specified in GraphletIdentifiersWithOrbits.pdf.\n");
