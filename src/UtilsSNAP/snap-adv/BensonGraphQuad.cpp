...
// A constructor
BensonGraphQuad::BensonGraphQuad() : BensonGraph(), motif(), orbits(0) {
}

// the real constructor
BensonGraphQuad::BensonGraphQuad(PNGraph& one_graph,MotifTypeQuad& one_motif,TIntV const& orbs, bool decomp, bool verb) : BensonGraph(one_graph,decomp,verb), motif(one_motif), orbits(orbs), ungraph(TSnap::ConvertGraph<PUNGraph>(one_graph)) {
  if (decomp){
  ...
  for (int cpt=0; cpt<w_match.Len();cpt++){
    ...
    for (int s=1;s<nbElts+1;s++){
      ...
      for (int t=s+1;t<nbElts+1;t++){ //(u_s,u_t,vi,w) may be a graphlet:
        ...
        TIntV orb_places(4);
        if (motif.IsInstance(id_st,orb_places)){

          TIntV lnodes(4);
          if (orbits.Len()==0){
            lnodes[0] = vi; lnodes[1] = w; lnodes[2] = u_s;lnodes[3] = u_t;
          }
          else{
            lnodes.Reserve(4,0);
...
BensonGraphQuadMP::BensonGraphQuadMP(PNGraph& one_graph,MotifTypeQuad& one_motif,TIntV const& orbs, const TInt& nThreads, bool verb) : BensonGraphQuad(one_graph,one_motif,orbs,false,verb), nbProc(nThreads) {
...
void BensonGraphQuadMP::IncrementWeight(TIntV const& lnodes) {
